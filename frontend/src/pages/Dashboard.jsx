...
function Toast({ text, type, onDone }) {
  ...
  return (
    <div className={`toast${type==="error"?" toast-error":""}`}>
      ...
      <button onClick={onDone} style={{ background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18 }}>✕</button>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const restartBot = async () => { await api("POST","/bot/restart"); showToast("Bot dimulakan semula..."); };
  ...
  const statusLabel = { connected:"Bersambung", qr_pending:"Imbas QR", starting:"Memulakan...", disconnected:"Terputus", auth_failed:"Auth Gagal" }[status.status]||"Terputus";
      ...
      <aside className="sidebar">
        ...
        <div style={{ margin:"10px 8px",background:"rgba(255,255,255,.05)",borderRadius:10,padding:"10px 12px",border:"1px solid rgba(255,255,255,.06)" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:8 }}>
            ...
            <span className={`badge ${statusBadge}`} style={{ fontSize:10 }}>{statusLabel}</span>
            {status.phone_number && <span style={{ fontSize:10,color:"#64748b",marginLeft:"auto" }}>+{status.phone_number}</span>}
          </div>
        ...
        <div className="sidebar-footer">
          <div style={{ color:"#64748b",fontSize:12,padding:"4px 12px",marginBottom:6,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
            👤 {user.name}
          </div>
            ...
            <div style={{ maxWidth:520,margin:"0 auto" }}>
              {status.status==="connected" && (
                <div className="card" style={{ textAlign:"center",padding:"32px 20px" }}>
                  <div style={{ width:64,height:64,borderRadius:16,background:"linear-gradient(135deg,#25d366,#128c5e)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:14,boxShadow:"0 8px 24px rgba(37,211,102,.3)" }}>✅</div>
                  <h2 style={{ fontWeight:800,fontSize:19,marginBottom:6 }}>WhatsApp Bersambung!</h2>
                  <p style={{ color:"#64748b",fontSize:14,marginBottom:4 }}>📱 +{status.phone_number}</p>
                  <p style={{ color:"#94a3b8",fontSize:13,marginBottom:22 }}>Bot aktif & menjawab mesej automatik</p>
              ...
              {status.status==="qr_pending" && status.qr_code && (
                <div className="card" style={{ textAlign:"center" }}>
                  <h2 style={{ fontWeight:800,fontSize:17,marginBottom:6 }}>Imbas Kod QR</h2>
                  <p style={{ color:"#64748b",fontSize:13,marginBottom:20 }}>WhatsApp → <b>Peranti Terpaut</b> → <b>Tautkan Peranti</b></p>
              ...
              {status.status==="starting" && (
                <div className="card" style={{ textAlign:"center",padding:"40px 20px" }}>
                  <span className="spinner spinner-dark" style={{ width:32,height:32,marginBottom:16 }} />
                  <h2 style={{ fontWeight:700,fontSize:16,marginBottom:8 }}>Sedang Memulakan...</h2>
              ...
              {(status.status==="disconnected"||status.status==="auth_failed") && !status.is_running && (
                <div className="card" style={{ textAlign:"center",padding:"40px 20px" }}>
                  ...
                  <h2 style={{ fontWeight:800,fontSize:19,marginBottom:8 }}>Bot Belum Aktif</h2>
          ...
          {tab===1 && config && (
            <div style={{ maxWidth:540,margin:"0 auto",display:"flex",flexDirection:"column",gap:14 }}>
              <div className="card">
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize:11,color:"#94a3b8",marginTop:5 }}>Pisahkan koma. Tanpa +</p>
                  </div>
                  <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",background:"#f8fafc",borderRadius:10,padding:"13px 14px",border:"1px solid #e2e8f0" }}>
                    <div>
                      <div style={{ fontWeight:600,fontSize:14 }}>Abaikan Group</div>
                      <div style={{ fontSize:12,color:"#94a3b8" }}>Bot tak balas mesej group</div>
                    </div>
              ...
              <button className="btn btn-primary" onClick={saveConfig} disabled={saving} style={{ width:"100%",padding:13 }}>
                {saving?<><span className="spinner" style={{ width:14,height:14 }}/> Menyimpan...</>:"💾 Simpan Tetapan"}
              </button>
            </div>
          )}
          ...
          {tab===3 && config && (
            <div style={{ maxWidth:500,margin:"0 auto",display:"flex",flexDirection:"column",gap:14 }}>
              <div className="card">
                ...
                <div className="section-sub">Bot hantar QR ini bila pelanggan tanya tentang bayaran</div>
              ...
              {logs.length===0 ? (
                ...
                <>
                  <div style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#f8fafc",borderRadius:10,marginBottom:10,border:"1px solid #e2e8f0" }}>
                    <input type="checkbox" checked={selectedLogs.size===logs.length&&logs.length>0} onChange={toggleAll} style={{ width:15,height:15,accentColor:"#25d366" }} />
                      ...
                      <div key={log.id} className="card" onClick={()=>toggleLog(log.id)}
                        ...
                        <div style={{ display:"flex",justifyContent:"space-between",marginBottom:10,gap:8 }}>
                          <div style={{ display:"flex",alignItems:"center",gap:8,minWidth:0 }}>
                            <input type="checkbox" checked={selectedLogs.has(log.id)} onChange={()=>toggleLog(log.id)} onClick={e=>e.stopPropagation()} style={{ width:14,height:14,accentColor:"#25d366",flexShrink:0 }} />
                          ...
                          <div style={{ display:"flex",alignItems:"center",gap:6,flexShrink:0 }}>
                            ...
                            <button className="btn btn-ghost btn-sm" style={{ padding:"3px 7px",color:"#ef4444",fontSize:13 }}
